...
NUMBER_OF_MAX_SEARCH_STEPS = 5  # Increased from 3 for better depth
MAX_SIMULATION_TIME = 3  # Increased from 2 for better quality
...
def predict(forest_map: ForestMap) -> List[Tuple[int, int]]:
    try:
        ...
        # First add fire brigades, then forester patrols – this ordering is
        # important and is assumed by FireSimulationNode (first N agents are
        # brigades, remaining are patrols).
        agents: List = []
        ...
        if not best_actions:
            logger.warning(f"MCTS: No actions found ({elapsed:.2f}s)")
            return []
        ...
        logger.info(f"MCTS: {len(fire_brigade_actions)}FB/{len(forester_patrol_actions)}FP, {elapsed:.2f}s, score={best_score:.3f}")
